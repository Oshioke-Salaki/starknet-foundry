    ...
    let result = rt
        .block_on(run(
            ...
            &[ForkTarget::new(
                ...
                RawForkParams {
                    url: node_url_with_version().unwrap().to_string(),
...
fn get_block_info_in_forked_block() {
    let test = test_case!(formatdoc!(
        r#"
            ...
            #[test]
            #[fork(url: "{node_url}", block_id: BlockId::Number(54060))]
        ...
        "#,
        node_url = node_url_with_version().unwrap()
    ).as_str(),
